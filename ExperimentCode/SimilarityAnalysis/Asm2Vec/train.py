...
@click.command()
@click.option('-i', '--input', 'ipath', default='d:/gu/dataset/train-asm/diffutils/gcc-o0/', help='training data folder', required=True)
@click.option('-o', '--output', 'opath', default='d:/gu/output/test.pt', help='output model path', show_default=True)
...
@click.option('-b', '--batch-size', 'batch_size', default=128, help='batch size', show_default=True)
@click.option('-e', '--epochs', default=100, help='training epochs', show_default=True)
@click.option('-n', '--neg-sample-num', 'neg_sample_num', default=25, help='negative sampling amount', show_default=True)
@click.option('-a', '--calculate-accuracy', 'calc_acc', default=True, help='whether calculate accuracy ( will be significantly slower )', is_flag=True)
