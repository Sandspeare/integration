...
class ASM2VEC(nn.Module):
    ...
    def v_pre_next_funcembed(self, inp):
    ...
    def forward(self, inp, pos, neg):
        ...
        v_batch = self.v_pre_next_funcembed(inp)
        # negative sampling loss
        pred = torch.bmm(self.embeddings_r(torch.cat([pos, neg], dim=1)), v_batch).squeeze()
        label = torch.cat([torch.ones(batch_size, 3), torch.zeros(batch_size, neg.shape[1])], dim=1).to(device)
        return bce(sigmoid(pred), label)#caculate the loss_total

    def predict(self, inp, pos):
        device, batch_size = inp.device, inp.shape[0]
        v = self.v_pre_next_funcembed(inp)
