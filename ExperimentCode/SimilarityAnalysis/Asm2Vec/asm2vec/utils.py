...
def load_data(paths, limit=None):
    ...
    #        filenames += [Path(path)]
    for path in paths:        
        filenames += find_all_files(path)
    
    functions, tokens = [], Tokens()
    print("loadding the dataset...")
...
#tensor x is functin_index  pre and next instruction token represent
#tensor y is the internal  instruction                 
...
def train(
    ...
    print("the mode is {0},trainning...".format(mode))
    for epoch in tqdm.tqdm(range(epochs)):
        ...
        for i, (inp, pos) in enumerate(loader):
            ...
            loss = model(inp.to(device), pos.to(device), neg.to(device)) # call ASM2VEC.forward
