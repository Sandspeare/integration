...
import re
class Function:
    def __init__(self, insts, blocks, meta, addrs):
        self.insts:List[Instruction] = insts
    ...
    def load(cls, text):
        ...
        for line in text.strip('\n').split('\n'):
            ...
            if 'call' in line:
                ...
                addrs.append(call_addr)

            # instruction             
            line = line.replace(' - ', ' + ')
            line = re.sub(r'0x[0-9a-f]+', 'CONST', line)
        ...
        return cls(insts, blocks, meta, addrs)
    def tokens(self):
